...
- **`TJwtUserContainer`** (`user_container_token`): Container authentication tokens
  ...
  - **Generated by**: Ganymede (via `POST /gateway/tokens/user-container`), not by gateway
...
| `POST` | `/gateway/tokens/scoped` | Generate project-scoped JWT token (gateway calls this) |

...
> {
    ...
    private async makeAirtableRequest(g: Ra<unknown>, endpoint: string, options: RequestInit = {}) {
    ...
    private async _fetchAndUpdateBase(
        ...
        try {
            ...
            const baseResponse = basesResponse.bases?.find((base: { id: string }) => base.id === baseId);
            ...
            const tablesResponse = await this.makeAirtableRequest(g, `/meta/bases/${baseId}/tables`);
            ...
            const tablesWithRecords = await Promise.all(
                newTables.map(async (table: { id: string; name: string; description?: string; fields: unknown[]; primaryFieldId: string; views: unknown[] }) => {
                    try {
                        const recordsResponse = await this.makeAirtableRequest(g, `/${baseId}/${table.id}`);
                        return {
                            ...table,
                            records: recordsResponse.records || [],
                        };
                    } catch (error) {
                        console.error(`Failed to fetch records for table ${table.id}:`, error);
                        return {
                            ...table,
                            records: [],
                        };
            ...
            if (existingBase) {
                ...
                const newTableIds = new Set(tablesWithRecords.map((t: TAirtableTable) => t.id));
                ...
                // Check for deleted records in remaining tables
                existingBase.tables.forEach((oldTable) => {
                    const newTable = tablesWithRecords.find((t: TAirtableTable) => t.id === oldTable.id);
                    if (newTable) {
                        const newRecordIds = new Set(newTable.records.map((r: TAirtableRecordValue) => r.id));
    ...
    private async _periodic(g: Ra<TEventPeriodic>): Promise<void> {
        ...
        if (timeSinceLastSync >= syncInterval) {
            ...
            // Sync all bases
            const baseIds = Array.from(g.sd.airtableBases.values()).map((b) => b.id);
            for (const baseId of baseIds) {
                try {
                    await this._fetchAndUpdateBase({ ...g, event: { baseId } });
                } catch (error) {
                    console.error(`Failed to sync base ${baseId}:`, error);
                }
            }
        }
    }
        ...
        try {
            ...
            if (base) {
                ...
                if (table) {
                    ...
                    if (record) {
                        record.fields = { ...(record.fields as object), ...fields } as Record<string, TJson>;
                    }
                }
            }
        } catch (error) {
    ...
    private async _createRecord(
        g: Ra<TEventCreateRecord>,
    ): Promise<void> {
        try {
            const { baseId, tableId, fields } = g.event;

            const response = await this.makeAirtableRequest(g, `/${baseId}/${tableId}`, {
                method: 'POST',
                body: JSON.stringify({ fields }),
            });
            ...
            // Update local state
            const base = g.sd.airtableBases.get(baseId);
            if (base && response.records && response.records.length > 0) {
                const table = base.tables.find((t) => t.id === tableId);
                if (table) {
                    table.records.push(response.records[0]);
                }
            }
        } catch (error) {
    ...
    private async _deleteRecord(
        ...
        try {
            ...
            // if the kanban column is now empty after this record deletion
            g.sd.nodes.forEach((node) => {
                if (node.type === 'airtable-kanban-column') {
                    ...
                    if (nodeData.baseId === baseId && nodeData.tableId === tableId) {
                        ...
                        if (base) {
                            const table = base.tables.find((t) => t.id === tableId);
                            if (table) {
                                const columnRecords = table.records.filter((r: TAirtableRecordValue) => {
    ...
    private async _reorderRecord(
        ...
        try {
            ...
            if (base) {
                const table = base.tables.find((t) => t.id === tableId);
                if (table) {
                    const orderField = table.fields.find((f) =>
                    ...
                    if (orderField) {
                        ...
                        const record = table.records.find((r) => r.id === recordId);
                        if (record) {
                            (record.fields as Record<string, unknown>)[orderField.id] = newPosition;
                        }
                    }
                }
            }
    ...
    private async _deleteRecordNode(g: Ra<TEventDeleteRecordNode>): Promise<void> {
        g.bep.process({
            type: 'core:delete-node',
            id: g.event.nodeId,
        });
    ...
    private async _deleteBase(
        ...
        try {
            ...
            g.sd.nodes.forEach((node) => {
                if (node.type === 'airtable-table' || node.type === 'airtable-record' || node.type === 'airtable-kanban-column') {
    ...
    private async _searchBases(g: Ra<TEventSearchBases>): Promise<void> {
        try {
            ...
            if (response.bases) {
                ...
                if (query) {
                    filteredBases = response.bases.filter((base: { name: string; description?: string }) =>
                        base.name.toLowerCase().includes(query.toLowerCase()) ||
                        (base.description && base.description.toLowerCase().includes(query.toLowerCase()))
                    );
                }

                const searchResults: TAirtableBaseSearchResult[] = filteredBases.map((base: { id: string; name: string; description?: string; url: string; permissionLevel: string }) => ({
                    id: base.id,
                    name: base.name,

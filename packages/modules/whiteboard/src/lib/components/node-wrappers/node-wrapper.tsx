...
import { useDebugComponent } from '@holistix-forge/ui-toolkit/frontend';
...
export const NodeWrapper =
  ...
  ({ id, data }: SpaceNode) => {
    ...
    const close = () =>
      dispatcher.dispatch({ type: 'whiteboard:close-node', nid: id, viewId });

    const open = () =>
      dispatcher.dispatch({ type: 'whiteboard:open-node', nid: id, viewId });

    const reduce = () =>
      dispatcher.dispatch({ type: 'whiteboard:reduce-node', nid: id, viewId });

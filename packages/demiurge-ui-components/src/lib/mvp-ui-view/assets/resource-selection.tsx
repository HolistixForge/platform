...
export const ResourceSelection = ({ isOpen }: ResourceButtonsProps) => {
  ...
  return (
    <div
      ...
      className={`cursor-pointer flex flex-row space-x-4 text-white h-[72px] min-w-[388px] ${isMenuOpen ? "rounded-b-none" : ""} rounded-lg pl-[38px] uppercase relative text-[20px] py-[10px] items-center justify-between`}
    >
      {ressources[selected].name}
      <div className="flex items-center h-full">
        <div className="h-full bg-white w-px" />
        <icons.ChevronDown
          className={`h-10 w-10 fill-white transition-transform origin-center ${isMenuOpen ? 'rotate-90' : ''}`}
        />
      </div>
      {isMenuOpen && (
        <div className="absolute w-full left-0 top-full !mx-0" onClick={() => {
          setIsMenuOpen(false);
        }}>
          {ressources.filter((ressource) => ressource.id !== selected).map((ressource) => (
            <div
              key={ressource.id}

...
export const setupUserRoutes = (router: Router) => {
  ...
  router.get('/users/search', authenticateJwt, asyncHandler(async (req: AuthRequest, res) => {
    const query = req.query.query as string;
    if (!query) {
      return res.status(400).json({ error: 'Query parameter required' });
    }

    const result = await pg.query('SELECT * FROM func_users_search($1)', [query]);
    const users = result.next()?.allRows() || [];
    return res.json({ users });
  }));

  // GET /users/:user_id - Get user by ID
  router.get('/users/:user_id', authenticateJwt, asyncHandler(async (req: AuthRequest, res) => {

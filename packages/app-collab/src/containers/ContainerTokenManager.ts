...
export class ContainerTokenManager {
  ...
  constructor(
    ...
    this.secret = secret || process.env.GATEWAY_HMAC_SECRET || 'default-secret-change-me';
   ...
   */
  validateToken(token: string): { container_id: string; project_id: string } | null {
    // Find container by token
    const data = this.gatewayState.getData();
    
    for (const [container_id, containerToken] of Object.entries(data.container_tokens)) {
      if (containerToken.token === token) {
        log(7, 'CONTAINER_TOKEN', `Token validated for container: ${container_id}`);
        return {
          container_id,
  ...
  getToken(container_id: string): string | null {
    const containerToken = this.gatewayState.getData().container_tokens[container_id];
  ...
  listTokens(project_id?: string): Array<{
    ...
    const data = this.gatewayState.getData();
    const tokens = Object.entries(data.container_tokens).map(([container_id, token]) => ({
      container_id,

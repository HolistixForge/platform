...
export class OAuthManager {
  ...
  addClient(client: TOAuthClient): void {
    ...
    log(7, 'OAUTH', `Added client: ${client.client_id} for container: ${client.container_id}`);
  }
  ...
  getCode(code_string: string): TOAuthCode | null {
    return this.gatewayState.getData().oauth_authorization_codes[code_string] || null;
  }
  ...
  saveToken(token: TOAuthToken): void {
    ...
    log(7, 'OAUTH', `Saved token: ${token.token_id} for user: ${token.user_id}`);
  }
  ...
  cleanupExpired(): void {
    ...
    this.gatewayState.updateData((data) => {
      ...
      for (const [code, codeData] of Object.entries(data.oauth_authorization_codes)) {

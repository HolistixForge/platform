...
export const setBackendEventProcessor = (processor: BackendEventProcessor<any>) => {
  bep = processor;
};
...
export let startProjectCollabCallback: ((config: TProjectConfig) => Promise<void>) | null = null;

export const setStartProjectCollabCallback = (callback: (config: TProjectConfig) => Promise<void>) => {
...
export const setupCollabRoutes = (router: Router) => {
  ...
  router.post('/collab/event', asyncHandler(async (req: Request, res) => {
    if (!bep) {
  ...
  router.post('/collab/start', asyncHandler(async (req: Request, res) => {

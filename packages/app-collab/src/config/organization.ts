...
export function loadOrganizationConfig(): TOrganizationConfig | null {
  ...
  if (process.env.ORGANIZATION_CONFIG) {
    try {
      ...
      log(6, 'CONFIG', `Loaded organization config from ORGANIZATION_CONFIG env var`);
      return config;
    } catch (e: any) {
  ...
  const configPath = process.env.ORGANIZATION_CONFIG_FILE || '/config/organization.json';
  if (fs.existsSync(configPath)) {
    try {
      ...
      log(3, 'CONFIG', `Failed to load config from ${configPath}: ${e.message}`);
    }
  }

  // Option 3: Create mock config for development
  if (process.env.NODE_ENV === 'development') {
    log(5, 'CONFIG', 'No organization config found - using mock config for development');
    return {

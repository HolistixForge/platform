...
export async function initializeGateway(
  ...
  log(6, 'GATEWAY_INIT', `Initializing gateway for organization: ${config.organization_id}`);
  ...
  log(6, 'GATEWAY_INIT', `Initialized permissions for ${permissionManager.getAllUsers().length} users`);
 ...
 */
export async function shutdownGateway(projectRooms: ProjectRoomsManager): Promise<void> {
